...
contract SushiMaker {
    ...
    constructor(IUniswapV2Factory _factory, address _bar, address _sushi, address _weth, address _checker, address _protocol) public {
        factory = _factory;
        sushi = _sushi;
        bar = _bar;
        weth = _weth;
    ...
    function _toWETH(address token) internal returns (uint256) {
        if (token == sushi) {
            ...
            _safeTransfer(token, factory.getPair(weth, sushi), (amount*25)/100);
            _fromSUSHI((amount*25)/100);
        ...
        if (token == weth) {
            ...
            _safeTransfer(token, protocol, (amount*25)/100);
            _safeTransfer(token, factory.getPair(weth, sushi), (amount*50)/100);
        ...
        _safeTransfer(weth, factory.getPair(weth, sushi), (amountOut*50)/100);
    ...
    function _fromSUSHI(uint256 amountIn) internal {
        IUniswapV2Pair pair = IUniswapV2Pair(factory.getPair(weth, sushi));
        (uint reserve0, uint reserve1,) = pair.getReserves();
        address token0 = pair.token0();
        (uint reserveIn, uint reserveOut) = token0 == sushi ? (reserve0, reserve1) : (reserve1, reserve0);
        uint amountInWithFee = amountIn.mul(995);
        uint numerator = amountInWithFee.mul(reserveOut);
        uint denominator = reserveIn.mul(1000).add(amountInWithFee);
        uint amountOut = numerator / denominator;
        (uint amount0Out, uint amount1Out) = token0 == sushi ? (uint(0), amountOut) : (amountOut, uint(0));
